...
class CFGTrainer:
    """Flow matching model trainer with classifier-free guidance."""
    ...
    def _train_epoch(self,
        ...
        model.train()
        iters = len(self.train_loader)
        for i, (x1, y) in enumerate(self.train_loader):
            x1 = x1.to(self.device)
            y = y.to(self.device)
            x0 = torch.randn_like(x1)
            t = torch.rand(len(x0)).to(self.device)
    ...
    def validate(self,
        ...
        model.eval()
        for i, (x1, y) in enumerate(self.val_loader):
            x1 = x1.to(self.device)
            y = y.to(self.device)
            x0 = torch.randn_like(x1)
            t = torch.rand(len(x0)).to(self.device)
