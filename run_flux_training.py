...
from warping.utils import load_json, prepare_filepaths_from_aligned_metadata, \
                          load_config
...
from warping.trainer import FluxTrainer, update_ema, requires_grad
...
def main(args):
    ...
    train_dict_lst = prepare_filepaths_from_aligned_metadata(metadata,
                                                             split='train')
    val_dict_lst = prepare_filepaths_from_aligned_metadata(metadata,
                                                           split='validation')

    train_dataset = AlignedDataset(emb_dict_lst=train_dict_lst,
                                   is_cache=True,
                                   seq_len=seq_len)
    ...
    if is_ema:
        ema = deepcopy(flux).to(device)  # Create an EMA of the model for use after training
        requires_grad(ema, False)
        update_ema(ema, flux, decay=0)  # Ensure EMA is initialized with synced weights
        ema.eval() # EMA model should always be in eval mode
    else:
        ema = None

    optimizer = torch.optim.AdamW(flux.parameters(),
                                  lr=learning_rate,
                                  weight_decay=weight_decay)
    scheduler = CosineAnnealingWarmRestarts(optimizer,
                                            num_epochs * len(train_loader),
                                            eta_min=0)
    trainer = FluxTrainer(interpolant=interpolant,
